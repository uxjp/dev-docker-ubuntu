...
doppler run -- bash -c 'docker run -it -p 8000:8000 --name github-ssh github-ssh bash -c "mkdir -p ~/.ssh && echo \"$ID_RSA_PUB_GITHUB\" > ~/.ssh/id_rsa.pub  && echo \"$ID_RSA_GITHUB\" > ~/.ssh/id_rsa && chmod 644 ~/.ssh/id_rsa.pub && chmod 600 ~/.ssh/id_rsa && ssh-keyscan github.com >> ~/.ssh/known_hosts && git config --global user.name $GITHUB_NAME && git config --global user.email $GITHUB_EMAIL && bash"'
